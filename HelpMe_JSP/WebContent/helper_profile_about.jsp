...
<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css">
	...
	<c:if test="${session_id eq null}">
		...
		<c:set var="helper" value="${requestScope.helperDetail}"/>	
		<c:set var="language" value="${requestScope.helperLanguage}"/>	
		<c:set var="specialty" value="${requestScope.helperSpecialty}"/>	
		<c:set var="languagesList" value="${requestScope.languagesList}"/>	
		<c:set var="servicesList" value="${requestScope.servicesList}"/>	
		...
		<section id="main" class="wrapper alt" style="padding-top:2rem">
				...
				<h5 class="aboutheader"><b>Specialties</b></h5>
					...
					<ul class="actions fit" id="${specialty.getSpecialized_field()}" style="margin-bottom:0px">
				<li>
					<select name="category" id="${specialty.getSpecialized_field()}" class="helperservice" style="background-color:white">
									<option value="${specialty.getSpecialized_field()}">${specialty.getSpecialized_field()}</option>
						</select>
					</li>
					
						
						<li>					
					<a href="javascript:deleteSpecialty('${specialty.getSpecialized_field()}')" id="removespecialty" class="button primary small" style="display:block">Remove</a>
						</li>
						</ul>						
					</c:forEach>
					</c:when>
		...
		<!-- Scripts -->
			...
			<script src="${pageContext.request.contextPath}/helper_profile_about.js"></script>
